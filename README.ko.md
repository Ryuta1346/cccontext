...
- 🎯 **예측적 인사이트**: 정교한 사용 패턴 분석을 통해 자동 압축 활성화 시점을 정확하게 예측
...
- 📊 **세션별 관리**: 각 세션의 토큰 사용량, 비용, 남은 용량을 개별적으로 표시
- ⚠️ **경고 시스템**: 컨텍스트 사용량이 일정 임계값에 도달할 때 경고
- 💰 **비용 계산**: 모델별 가격에 기반한 실시간 비용 계산
- 🎯 **비침입적**: Claude Code 자체에 영향을 주지 않고 JSONL 로그만 읽음
- 🤖 **자동 압축 추적**: Claude Code 자동 압축 활성화까지의 남은 용량 표시
...
Claude Code는 컨텍스트 창 사용량이 일정 임계값에 도달하면 자동으로 자동 압축을 실행하여 대화를 압축합니다. CCContext는 실제 Claude Code 동작과 일치하는 계산 방법을 사용하여 자동 압축 활성화 타이밍을 정확히 예측합니다.
