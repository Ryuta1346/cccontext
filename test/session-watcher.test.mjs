...
describe("SessionWatcher", () => {
  ...
  describe("Error Cases", () => {
    ...
    it("should handle malformed JSONL files", async () => {
      ...
      const malformedContent = ["not json", "{ invalid json", '{"valid": "json"}', "null", ""].join("\n");
  ...
  describe("Compact Handling", () => {
    ...
    it("should handle file size decrease (simulating /compact)", async () => {
      ...
      await fs.writeFile(sessionFile, initialData.map((d) => JSON.stringify(d)).join("\n"));
      ...
      await fs.writeFile(sessionFile, compactedData.map((d) => JSON.stringify(d)).join("\n"));
    ...
    it("should handle large file size changes", async () => {
      ...
      await fs.writeFile(sessionFile, largeData.map((d) => JSON.stringify(d)).join("\n"));
