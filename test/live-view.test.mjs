...
vi.mock("blessed", () => ({
  default: {
    ...
    message: vi.fn((options) => ({
      ...
      display: vi.fn(),
      destroy: vi.fn(),
      error: vi.fn((_msg, callback) => callback()),
    })),
  },
}));
...
describe("LiveView", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should set up key bindings", () => {
      ...
      const quitBinding = keyBindings.find((call) => call[0].includes("q") && call[0].includes("C-c"));
      expect(quitBinding).toBeTruthy();

      // Check r binding
      const refreshBinding = keyBindings.find((call) => call[0].includes("r"));
  ...
  describe("Formatting", () => {
    ...
    it("should format latest prompt with truncation", () => {
      const info = {
        latestPrompt:
          "This is a very long prompt that should be truncated after a certain number of lines to fit in the display properly without taking up too much space",
      };
  ...
  describe("Warning Levels", () => {
    ...
    it("should display appropriate warning messages", () => {
      ...
      expect(view.getWarningMessage({ warningLevel: "warning" })).toContain("High context usage");
      expect(view.getWarningMessage({ warningLevel: "severe" })).toContain("Approaching context limit");
      expect(view.getWarningMessage({ warningLevel: "critical" })).toContain("CRITICAL");
    });
  ...
  describe("Keyboard Controls", () => {
    ...
    it("should exit on q key", () => {
      const quitHandler = view.screen.key.mock.calls.find((call) => call[0].includes("q"))[1];
    ...
    it("should refresh on r key", () => {
      ...
      const refreshHandler = view.screen.key.mock.calls.find((call) => call[0].includes("r"))[1];
  ...
  describe("Messages and Errors Display", () => {
    ...
    it("should show regular messages", () => {
      ...
      expect(view.boxes.statusBar.setContent).toHaveBeenCalledWith(expect.stringContaining("Test message"));
      expect(view.screen.render).toHaveBeenCalled();
    });
