...
describe("EnhancedSessionsManager", () => {
  ...
  describe("Session Loading", () => {
    ...
    it("should handle session loading errors gracefully", async () => {
      ...
      await fs.writeFile(path.join(projectDir, "invalid.jsonl"), "invalid json content");

      // Mock getAllJsonlFiles to return the invalid file
      manager.watcher.getAllJsonlFiles = vi.fn(async () => [path.join(projectDir, "invalid.jsonl")]);
